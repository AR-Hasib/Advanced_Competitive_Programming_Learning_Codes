...
struct Tree {
    ...
    void addPath(int u, int v){
        ...
        int lc = getLCA(u, v);
        vec[lc]--;
        int x = getKthAncestor(lc, 1);
        vec[x]--; 
    }

    void computeAns(int node, int parent){    
        for(int &child : adj[node]){
            if(child != parent){
...
signed main(){
    ...
    // we will keep a vector and the operation we will do as follow
    //  vec[u]++, vec[v]++, vec[lca(u, v)]--, vec[parent(lca(u, v))]--
    while(m--){
        int u, v; cin>>u>>v;
        tree.addPath(u, v);
    }
